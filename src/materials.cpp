...
void Materials::xml_add_material(pugi::xml_node node) {
    ...
    nameofn.append_child(pugi::node_pcdata)
        .set_value(join(namenuclides, " ").c_str());
    auto concpair = usplit(this->conc);
    auto concnod = material.append_child("conc");
    concnod.append_child(pugi::node_pcdata)
        .set_value(joinDouble(concpair.first, " ").c_str());
    auto dconcnod = material.append_child("dconc");
    dconcnod.append_child(pugi::node_pcdata)
        .set_value(joinDouble(concpair.second, " ").c_str());
}

void Materials::add_nuclide(const std::string& extname, udouble extconc) {
    auto it = std::find(this->namenuclides.begin(), this->namenuclides.end(),
                        extname);
...
void read_materials_from_inp(std::string inp_path) {
    ...
    for (pugi::xml_node tool : root_node.children("material")) {
        ...
        auto m = new Materials(mname, atof(tool.attribute("volume").value()),
        ...
        std::for_each(
            m->namenuclides.begin(), m->namenuclides.end(),
            [m](std::string& nucname) {
                m->indexnuclides.push_back(get_nuclidarray_index(nucname));
            });
...
extern "C" int openbps_material_get_nuclides_by_idx(int32_t index, char*** nuclides) {
    int err = 0;
    if (index >= 0 && index < openbps::materials.size()) {
        try {
            size_t i = 0;
...
extern "C" int openbps_material_get_conc_by_idx(int32_t index, double** real, double** dev) {
    int err = 0;
    if (index >= 0 && index < openbps::materials.size()) {
        try {
            size_t i = 0;
...
extern "C" int openbps_material_get_idx_nuclides_by_idx(int32_t index, int** index_nuclides) {
    int err = 0;
    if (index >= 0 && index < openbps::materials.size()) {
        try {
            size_t i = 0;
            for (auto& el : openbps::materials[index]->indexnuclides) {
                *index_nuclides[i] = el;
                i++;
            }
        } catch (const std::runtime_error& e) {
